 ...
 */
function setupEnvironment(inputs) {
 ...
 */
async function processTemplate(inputs) {
 ...
 */
async function runAugmentScript(inputs) {
    ...
    const uniqueMcpConfigs = Array.from(new Set(inputs.mcpConfigs ?? [])).filter(config => config.length > 0);
