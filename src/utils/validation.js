...
const createJsonStringArraySchema = (invalidTypeMessage, emptyEntryMessage) => z.preprocess(value => {
    ...
    .refine(val => val.length > 0, { message: emptyEntryMessage }), { invalid_type_error: invalidTypeMessage })
    .optional());
/**
...
const ActionInputsSchema = z
    ...
    repoName: z
        ...
        .optional(),
    rules: createJsonStringArraySchema('rules must be a JSON array of strings', 'Rule file paths cannot be empty'),
    mcpConfigs: createJsonStringArraySchema('mcp_configs must be a JSON array of strings', 'MCP config file paths cannot be empty'),
})
    .refine(data => {
    ...
    message: 'Cannot use both augment_session_auth and augment_api_token/augment_api_url simultaneously',
    path: ['augmentSessionAuth', 'augmentApiToken', 'augmentApiUrl'],
})
    .refine(data => {
    if (!data.augmentSessionAuth)
        return true;
    try {
...
export class ValidationUtils {
    ...
    static validateInputs() {
        try {
            ...
            const inputs = Object.fromEntries(Object.entries(INPUT_FIELD_MAP)
                .map(([key, fieldDef]) => {
                ...
                const transformedValue = fieldDef.transform && value ? fieldDef.transform(value) : value;
                return [key, transformedValue];
            })
                .filter((entry) => entry !== null));
            logger.debug('Validating action inputs');
            const validated = ActionInputsSchema.parse(inputs);
